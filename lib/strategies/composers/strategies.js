...
function setContext(name, propertyName, strategy, arrayMode, Base) {
  return function(context, ...strategyArgs) {
    ...
    const isImport = name.substr(0, 6) === 'import';
    const ref = isImport ? raw[propertyName] : res[propertyName];
    ...
    if (arrayMode)
      res[propertyName] = isImport && _.size(ref || []) ? [] : res[propertyName] || [];
    else
      res[propertyName] = res[propertyName] || new (Base || Object)();
