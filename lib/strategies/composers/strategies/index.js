...
export function strategies(acc, property, propertyName, subActions) {
  ...
  _.each(allStrategies, strategy => {
    ...
    if (subStrategy)
      orderedStrategies.push(subStrategy.$notContextualized || subStrategy);
    ...
    acc.addToBuild(strategy.fullName, setContext({
      ...
      strategy: function(context, ...strategyArgs) {
        ...
        _.each(orderedStrategies, (strategy, index) => {
          if (index)
            prom = prom.tap(modifyAttrVal(context.$propertyName).bind(null, context.$parent));
          prom = prom.then(() => strategy(context, ...strategyArgs));
        });
        return prom;
      },
