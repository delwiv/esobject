...
exports.default = {
  ESObjectError: {
    ...
    IndexConflict: 'Trying to {{fn}} an object from a different index ' + '({{obj._index}}/{{obj._type}}/{{obj._id || "<no id>"}}) in index {{name}}',
    ClientConflict: 'Impossible to add an index that does not share the same client that other indexes (for now).',
    TypeAgain: 'Impossible to add a type with name {{name}} in this store because there is alreay one ' + 'with this name inside.',
  ...
  ESLoadError: {
    BadRequire: 'failed to load file {{file}} ' + '{{? if (from) {}}(loaded from {{from}}) {{? } }}' + 'with message: {{message}}.',
    BadYamlRequireProp: '!require with file {{require}} and prop {{prop}} should not resolve to undefined ' + '(from yaml {{file}}).',
