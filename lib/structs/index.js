import {ESObjectError, ESTypeError, getStackedToESError} from '../errors';
...
function decorateESArray(elts, Types) {
  var strategies = _.reduce(Types, function(acc, Type) {
    ...
    while (!ESType.strategies)
      ESType = Type.super_;

    var strategies = _.keys(ESType.strategies);
  ...
  return Promise.resolve(this.$client[action](_.extend({
    ...
    .then(function(data) {
      var res = _.map((data.hits || {}).hits || [], data => {
        var Type = _.find(Types, Type => Type.prototype._type === data._type);
